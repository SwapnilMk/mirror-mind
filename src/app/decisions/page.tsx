        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-[#13131e] border border-white/10 w-full max-w-lg rounded-[28px] overflow-hidden shadow-2xl relative animate-in zoom-in-95 duration-200 z-10 flex flex-col max-h-[85vh]">
            ...
            <div className="p-6 border-b border-white/6 flex items-start justify-between gap-4">
              <div>
                <span className={`inline-block text-[10px] font-semibold px-2 py-0.5 rounded-full mb-2 ${catColors[selectedDecision?.category] || "bg-zinc-800 text-zinc-400"}`}>
                ...
                <>
                  ...
                  <div className="flex items-center justify-between text-xs text-zinc-500 bg-white/2 p-3 rounded-xl border border-white/4">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={`font-bold uppercase tracking-widest ${
                        selectedDecision.outcome === "good" ? "text-emerald-400" : 
                        selectedDecision.outcome === "bad" ? "text-rose-400" : 
                        "text-amber-500"
                      }`}>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      <h4 className="text-zinc-500 text-xs font-semibold uppercase tracking-wider">Confidence Level</h4>
                      <span className="text-zinc-400 font-mono text-xs">{selectedDecision.confidence}%</span>
                    </div>
                    <div className="h-2 bg-white/6 rounded-full overflow-hidden">
                      <div className="h-full bg-gradient-to-r from-violet-600 to-purple-500 rounded-full" style={{ width: `${selectedDecision.confidence}%` }} />
                    </div>
                  </div>
                  ...
                  {selectedDecision.reasoning && (
                    <div className="relative mt-2">
                      <h4 className="text-zinc-500 text-xs font-semibold uppercase tracking-wider mb-1.5 flex items-center gap-1">
                        <Brain size={13} className="text-violet-400 animate-pulse" /> Cognitive Reasoning
                      </h4>
                  ...
                  <div className="bg-[#1a1a27] border border-white/6 rounded-2xl p-4 flex flex-col gap-4 mt-2">
                    ...
                    <div>
                      <label className="text-[11px] text-zinc-400 font-medium mb-2 block">Outcome State</label>
                        ...
                        {["pending", "good", "bad"].map(opt => (
                          <button
                            ...
                            className={`flex-1 py-2 rounded-xl text-xs font-bold uppercase tracking-wider border transition-all cursor-pointer ${
                              editOutcome === opt
                                ? opt === "good" ? "bg-emerald-600/20 border-emerald-500 text-emerald-400 shadow-md shadow-emerald-950/20"
                                  : opt === "bad" ? "bg-rose-600/20 border-rose-500 text-rose-400 shadow-md shadow-rose-950/20"
                    ...
                    {editOutcome !== "pending" && (
                      <div>
                        <div className="flex items-center justify-between mb-1.5">
                          <label className="text-[11px] text-zinc-400 font-medium">Regret Score</label>
                          <span className="text-violet-400 text-xs font-bold font-mono">{editRegret}/10</span>
                        </div>
                        <input
                          type="range"
                          min={0}
                        ...
                        <div className="flex justify-between text-zinc-600 text-[9px] mt-1">
                          <span>No regrets (0)</span><span>Deep regret (10)</span>
                        </div>
                      </div>
                    )}

                    <div>
                      <label className="text-[11px] text-zinc-400 font-medium mb-2 block">Current Emotion / Mood</label>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <label className="text-[11px] text-zinc-400 font-medium">Perceived Stress Level</label>
                        <span className="text-violet-400 text-xs font-bold font-mono">{editStress}/10</span>
                      </div>
                      <input
                        type="range"
                        min={1}
