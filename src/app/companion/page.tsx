...
export default function CompanionPage() {
  ...
  const parseTaggedMarkdown = (content: string) => {
    ...
    return {
      ...
      insights: insightsMatch ? insightsMatch[1].split('|').map(s => s.trim()).filter(Boolean) : [],
      patterns: patternsMatch ? patternsMatch[1].split('|').map(s => s.trim()).filter(Boolean) : [],
      suggestions: suggestionsMatch ? suggestionsMatch[1].split('|').map(s => s.trim()).filter(Boolean) : [],
      reply: replyMatch ? replyMatch[1].trim() : content.replace(/\[(?:EMOTION|TITLE|INSIGHTS|PATTERNS|SUGGESTIONS)\][^\n]*/gi, "").trim(),
    }
  }
  ...
  const handleSend = async (textToSend: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { id: `assistant-temp-${Date.now()}`, role: "assistant", content: "", createdAt: new Date().toISOString() }])
  ...
  const getStageTitle = (stage: number) => {
    switch (stage) {
      case 1: return "Warm Introduction"
      case 2: return "Identity & Career"
      case 3: return "Personality & Emotion"
      case 4: return "Behavioral Loops"
      case 5: return "Life Timeline"
      default: return "Cognitive Double Synced"
    }
  }
  ...
  const getSuggestions = () => {
    ...
    if (stage === 1) {
      return ["I'm ready. Let's do this!", "My name is Swapnil.", "What kind of details do you need?"]
    }
    if (stage === 2) {
      return ["I work as a Software Developer.", "I spend my time coding and building products.", "My daily routine is quite structured."]
    }
    if (stage === 3) {
      return ["Financial risk makes me nervous.", "I overthink failure and career shifts.", "I feel motivated by creating unique platforms."]
    }
    if (stage === 4) {
      return ["I confront problems immediately.", "I avoid hard decisions to bypass conflict.", "I take calculated risks with backup plans."]
    }
    if (stage === 5) {
      return ["My proudest achievement was building this app.", "I regret not taking an entrepreneurial risk sooner.", "My biggest turning point was moving cities."]
    }
    return [
      ...
      <div className="space-y-6">
              ...
              <p className="text-[10px] text-zinc-400 flex items-center gap-1 mt-0.5">
                {profile.onboardingStage >= 6 ? (
                  ...
                  <>
                    ...
                    <span className="text-violet-400 font-medium">Stage {profile.onboardingStage} of 5 Mapping</span>
                  </>
                )}
              </p>
            </div>
        ...
        <div className="space-y-3">
          ...
          <div className="bg-[#0e0e1a]/80 border border-white/4 rounded-xl p-3.5 space-y-2.5 text-xs">
            <div className="flex justify-between"><span className="text-zinc-500">Name</span> <span className="text-zinc-200 font-medium">{profile.name || "Awaiting..."}</span></div>
            <div className="flex justify-between"><span className="text-zinc-500">Nickname</span> <span className="text-zinc-200 font-medium">{profile.preferredName || "Awaiting..."}</span></div>
            <div className="flex justify-between"><span className="text-zinc-500">Age</span> <span className="text-zinc-200 font-medium">{profile.age || "Awaiting..."}</span></div>
            <div className="flex justify-between"><span className="text-zinc-500">Birthday</span> <span className="text-zinc-200 font-medium">{profile.birthday || "Awaiting..."}</span></div>
            <div className="flex justify-between"><span className="text-zinc-500">Country</span> <span className="text-zinc-200 font-medium">{profile.country || "Awaiting..."}</span></div>
            <div className="flex justify-between"><span className="text-zinc-500">Language</span> <span className="text-zinc-200 font-medium">{profile.language || "Awaiting..."}</span></div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="bg-[#0e0e1a]/80 border border-white/4 rounded-xl p-3.5 space-y-2.5 text-xs">
            <div className="flex justify-between"><span className="text-zinc-500">Profession</span> <span className="text-zinc-200 font-medium">{profile.profession || "Awaiting..."}</span></div>
            <div className="flex justify-between"><span className="text-zinc-500">Company</span> <span className="text-zinc-200 font-medium">{profile.company || "Awaiting..."}</span></div>
            <div className="flex justify-between"><span className="text-zinc-500">Designation</span> <span className="text-zinc-200 font-medium">{profile.designation || "Awaiting..."}</span></div>
            <div className="flex justify-between"><span className="text-zinc-500">Work Style</span> <span className="text-zinc-200 font-medium">{profile.workStyle || "Awaiting..."}</span></div>
            <div className="flex justify-between"><span className="text-zinc-500">Risk Profile</span> <span className="text-violet-400 font-bold capitalize">{profile.riskStyle || "Awaiting..."}</span></div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-3">
            <div>
              <span className="text-[10px] text-zinc-500 font-bold block mb-1">Ambitions & Goals</span>
              <div className="flex flex-wrap gap-1">
                {profile.goals.length > 0 ? (
                  profile.goals.map((g, i) => (
                    <span key={i} className="bg-emerald-950/40 border border-emerald-900/30 text-emerald-400 text-[10px] px-2 py-0.5 rounded-md">
                      {g}
                    </span>
                  ))
                ) : (
            ...
            <div>
              <span className="text-[10px] text-zinc-500 font-bold block mb-1">Fears & Stressors</span>
              <div className="flex flex-wrap gap-1">
                {profile.fears.length > 0 ? (
                  profile.fears.map((f, i) => (
                    <span key={i} className="bg-red-950/40 border border-red-900/30 text-red-400 text-[10px] px-2 py-0.5 rounded-md">
                      {f}
                    </span>
                  ))
                ) : (
            ...
            <div>
              <span className="text-[10px] text-zinc-500 font-bold block mb-1">Overthinking Triggers</span>
              <div className="flex flex-wrap gap-1">
                {profile.overthinkItems.length > 0 ? (
                  profile.overthinkItems.map((o, i) => (
                    <span key={i} className="bg-purple-950/40 border border-purple-900/30 text-purple-400 text-[10px] px-2 py-0.5 rounded-md">
                      {o}
                    </span>
                  ))
                ) : (
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {episodics.length > 0 ? (
              episodics.map(e => (
                <div key={e.id} className="bg-[#0c0c16] border border-white/5 rounded-xl p-3 relative overflow-hidden group">
              ...
              <div className="bg-[#0c0c16] border border-white/5 rounded-xl p-4 text-center">
                <p className="text-zinc-600 text-xs italic">No milestones saved yet. Epinodics are scored (importance &gt;= 6) during conversation.</p>
              </div>
            )}
          </div>
        </div>
      ...
      <aside className="hidden lg:block lg:col-span-4 bg-[#0d0d17] border-r border-white/6 overflow-y-auto p-6 h-full select-none scrollbar-none">
        <div className="flex items-center gap-3 pb-5 mb-5 border-b border-white/6 sticky top-0 bg-[#0d0d17] z-10">
          ...
          <div>
            ...
            <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold">Memory & Profiles</p>
          </div>
        </div>
        {renderProfilePanel()}
      </aside>
      ...
      <section className="col-span-12 lg:col-span-8 flex flex-col h-full relative overflow-hidden bg-[#080810]">
                        ...
                        <div className="flex flex-col gap-3">
                          {parsed?.title && (
                            <div className="flex items-center gap-2 border-b border-white/10 pb-2 mb-1">
                              ...
                              <span className="font-bold text-violet-300 uppercase tracking-widest text-[10px]">{parsed.title}</span>
                              {parsed.emotion && (
                          ...
                          {(parsed?.insights?.length || parsed?.patterns?.length) ? (
                            <div className="flex flex-wrap gap-2 mt-2 pt-2 border-t border-white/5">
                              {parsed.insights?.map((insight, idx) => (
                                <span key={`i-${idx}`} className="bg-emerald-900/20 border border-emerald-500/20 text-emerald-400 text-[10px] px-2 py-1 rounded-md flex items-center gap-1.5">
                                  <Info size={10} /> {insight}
                                </span>
                              ))}
                              {parsed.patterns?.map((pattern, idx) => (
                                <span key={`p-${idx}`} className="bg-orange-900/20 border border-orange-500/20 text-orange-400 text-[10px] px-2 py-1 rounded-md flex items-center gap-1.5">
        ...
        {profile && !sending && !loading && (
          <div className="px-5 pb-3 flex gap-2 overflow-x-auto scrollbar-none shrink-0 mask-gradient-r">
            {(dynamicSuggestions.length > 0 ? dynamicSuggestions : getSuggestions()).map((suggestion, idx) => (
              <button
                key={idx}
            ...
            {profile.onboardingStage >= 6 && !(dynamicSuggestions.length > 0 ? dynamicSuggestions : getSuggestions()).some(s => s.includes("Dashboard") || s.includes("Go to")) && (
              <button
          ...
          >
            ...
            <div className="relative flex-1">
              <input
                ...
                placeholder={profile && profile.onboardingStage < 6 ? "Answer MirrorMind's inquiry..." : "Chat with your double..."}
